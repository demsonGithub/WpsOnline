...
using WPSOnline.Api.Model;

namespace WPSOnline.Api.Controllers
{
    [ApiController]
    public class WpsController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<GenarateResult> GenarateWPSUrl(GenarateRequest request)
        {
            return await Task.Run(() =>
            {
                var url = WPSSignatureHelper.GenerateUrl(request.FileId,
                                                request.FileType,
        ...
        public async Task<FileInfoResult> fileInfo()
        {
            ...
            {
                // 1. 过滤参数，判断是否可用
                var requestParam = FilterRequestForWPS(Request);
                ...
                // 2. 获取源文件,信息从数据库查询
                var nowStamp = TimestampHelper.GetCurrentTimestamp();

                WPSFile fileInfo = new WPSFile()
                {
                ...
                UserForFile userInfo = new UserForFile()
                {
                    id = "test_user_id",
                    name = "测试用户",
                    permission = "read",// write为允许编辑，read为只能查看
                    avatar_url = ""
                };
        ...
        [HttpPost]
        public async Task<UserModel> GetUserInfo(GetUserInfoRequest body)
        {
            return await Task.Run(() =>
            {
        ...
        [HttpPost]
        public async Task<WPSBaseModel> Online(GetUserInfoRequest body)
        {
            return await Task.Run(() =>
            {
        ...
        [HttpPost]
        public async Task<SaveFileResult> SaveFile(IFormFile file)
        {
        ...
        [HttpGet]
        public async Task<GetFileByVersionResult> Version(int version)
        {
            return await Task.Run(() =>
            {
                var request = FilterRequestForWPS(Request);
        ...
        [HttpPut]
        public async Task<WPSBaseModel> RenameFile(RenameFileRequest body)
        {
            return await Task.Run(() =>
            {
                var request = FilterRequestForWPS(Request);
        ...
        [HttpPost]
        public async Task<GetHistoryResult> GetHistory(GetHistoryRequest body)
        {
            return await Task.Run(() =>
            {
                var request = FilterRequestForWPS(Request);
        ...
        [HttpPost]
        public async Task<CreateWPSFileResult> NewFile(CreateWPSFileRequest request)
        {
            return await Task.Run(async () =>
            {
                try
